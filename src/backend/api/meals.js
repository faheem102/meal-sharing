  ...
  try {
    ...
    if ("limit" in request.query) {
      ...
      if (isNaN(limitAmount)) {
        response
          .status(404)
          .end("Wrong value : please enter a valid integer value");
      } else {
    ...
    if ("maxPrice" in request.query) {
      ...
      if (isNaN(maxPriceInteger)) {
        response
          .status(404)
          .end("Wrong value : please enter a Price in valid integer");
      }
      data = await knex("meal")
        .select("*")
        .where("price", "<", `${maxPriceInteger}`);
    }
    ...
    if ("availableReservations" in request.query) {
      const joinData = await knex("meal")
        .select(
          "title",
          "reservation.meal_id",
      ...
      const data = joinData.filter(
        (meal) => meal.number_of_guests < meal.max_reservations
      );
    }
...
router.delete("/:id", async (request, response) => {
  try {
    ...
    response.send("meal has been deleted");
  } catch (error) {
    throw error;
  }
});
